            ...
            options = {
              {
                type = "client",
                action = function()
                    TriggerEvent('tcrp-barber:client:menu',v.name)
                end,
...
RegisterNetEvent("tcrp-barber:client:menu",function()
    MainMenu()
end)
...
local MainMenus = {
    ...
    ["save"] = function()
        ...
        if IsPedMale(ped) then 
            TriggerServerEvent("tcrp-barber:SaveSkinm", CreatorCache)
        else
            TriggerServerEvent("tcrp-barber:SaveSkinf", CreatorCache)
        end
    end
}
...
function MainMenu(Target)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'main_character_creator_menu',
        {title = "Barber Shop", subtext = Config.Texts.Options, align = Config.Texts.align, elements = elements}, function(data, menu)
